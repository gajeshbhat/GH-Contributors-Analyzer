...
def get_api_response_default() -> object:
    return {"message": "Not found!"}


def get_topic_detail_formatted(topic, repo_list):
    topic_info = {
...
def process_star_count(number_of_stars):
    total_stars = 0
...
def get_slice_list(max_value, each_slice):
    list_slices = list()
...
def get_response(request_url, headers=None):
    response_value = None
    if headers is None:
    ...
    try:
        ...
        log_message(
            str("Error for URL " + request_url + response_value.reason + "\n" + str(traceback.format_exc())))
    return None


def find_all_elements(page_soup, tag, attribute_name, attribute_value):
    return page_soup.find_all(tag, {attribute_name: attribute_value})


def find_next_element_url(entry, tag):
    return entry.find_next(tag)['href']


def find_next_element(page_soup, tag, attribute_name, attribute_value):
    return page_soup.find_next(tag, {attribute_name: attribute_value})


def is_summary_response_none(summary_response):
    return summary_response is None


def get_repo_lang(raw_repo_soup):
    repo_lang = ''
    try:
...
def get_repo_star_count(raw_repo_soup):
    stargazer_soup = find_next_element(raw_repo_soup, 'a', 'class', 'd-inline-block link-gray')
    stargazer_count = 0
    stargazer_link = ''
    if stargazer_soup is None:
        stargazer_count = int(0)
        stargazer_link = 'N/A'
    else:
        stargazer_count = process_star_count(str(stargazer_soup.text))
...
class GithubParser():
    ...
    def __init__(self):
        ...
        self.github_crawl_details = {
            ...
            "topics_fetch_urls": ["https://github.com/topics",
                                  "https://github.com/topics?after=Y3Vyc29yOnYyOpKmc2tldGNozTHp"],
            "page_layout": {
                "all_elements_parse": {"tag": 'p', "attribute_name": 'class', "attribute_value": 'py-4 border-bottom'},
                "each_element_parse": {"tag": 'p', "attribute_name": 'class',
                                       "attribute_value_topic_name": 'f3 lh-condensed mb-0 mt-1 link-gray-dark',
    ...
    def get_each_topic_detail(self, topic_entry):
        topic_dict = {
            'topic_name': find_next_element(topic_entry, 'p', 'class',
                                            'f3 lh-condensed mb-0 mt-1 link-gray-dark').text,
            'topic_url': str("https://www.github.com" + find_next_element_url(topic_entry, 'a')).strip(),
            'description': str(find_next_element(topic_entry, 'p', 'class', 'f5 text-gray mb-0 mt-1').text).strip()
        }
        return topic_dict

    def get_topics_details(self, page_soup):
        all_topics_details = find_all_elements(page_soup, 'li', 'class', 'py-4 border-bottom')
    ...
    def get_topics(self):
        ...
        for summary_page in self.github_crawl_details['topics_fetch_urls']:
            summary_response = get_response(summary_page)
            if is_summary_response_none(summary_response):
                log_message("Request failed!" + "for URL " + summary_page)
                return all_topics
    ...
    def get_related_tag_list(self, raw_repo_soup):
        related_tag_column_soup = find_next_element(raw_repo_soup, 'div', 'class',
                                                    'topics-row-container d-flex flex-wrap flex-items-center f6 mb-3')
        related_tag_list_soup = find_all_elements(related_tag_column_soup, 'a', 'class',
                                                  'topic-tag topic-tag-link f6 my-1')
    ...
    def get_repo_basic_info(self, repo_page_soup):
        ...
        for info in all_repo_soup:
            basic_info_span = find_next_element(info, 'span', 'class', 'num text-emphasized')
    ...
    def get_repo_basic_desc(self, repo_page_soup):
        desc_data = ''
        if repo_page_soup.find('span', {'itemprop': 'about'}) is None:
            desc_data = 'N/A'
        else:
    ...
    def get_gen_repo_info(self, repo_url):
        ...
        repo_basic_details_response = get_response(repo_url)
    ...
    def get_top_contributor_list(self, url):
        ...
        api_response = get_response(str(self.GITHUB_API_URL_PREFIX + url + self.GITHUB_SITE_URL_POSTFIX), headers)
        self.MAX_HOUR_RATE_LIMIT -= 1
        if api_response is None:
            return json.loads(get_api_response_default())
        return json.loads(api_response.content)

    def get_clean_repo_info(self, raw_repo_soup):
        repo_clean_info = {
            'raw_repo_link': find_next_element_url(raw_repo_soup, 'a'),
            'repo_description': str(
                find_next_element(raw_repo_soup, 'div', 'class', 'text-gray mb-3 ws-normal').text).strip(),
            'repo_dominant_language': get_repo_lang(raw_repo_soup),
            'related_tag_list': self.get_related_tag_list(raw_repo_soup),
            'number_of_stars': get_repo_star_count(raw_repo_soup),
            'general_repo_info': self.get_gen_repo_info(str(find_next_element_url(raw_repo_soup, 'a'))),
            'repo_contributors': self.get_top_contributor_list(str(find_next_element_url(raw_repo_soup, 'a'))),
        }
        return repo_clean_info

    def get_all_repo_info(self, topic_page_soup):
        repo_list_soup = find_all_elements(topic_page_soup, 'article', 'class',
                                           'border-bottom border-gray-light py-4')
    ...
    def get_repos_of_topic(self, start_set, end_set):
        ...
        for topic in all_topics_details[start_set:end_set]:
            topic_page_response = get_response(str(topic['topic_url']))
            ...
            topic_details_formatted = get_topic_detail_formatted(topic, self.get_all_repo_info(topics_page_soup))
            db_obj.insert_item_github(topic_details_formatted)

    def parse_full_data(self):
        slice_list = get_slice_list(self.MAX_TOPIC_ENTRIES, self.EACH_SLICE_SIZE)
